...
public class SkCarRouteClient {
    ...
    private static final String CAR_PATH = "/tmap/routes?version=1";
    ...
    public Map<String, Object> route(
        ...
        Preconditions.validate(
                StringUtils.hasText(properties.appKey()), ErrorCode.SK_CAR_API_KEY_MISSING);

        String url = properties.baseUrl() + CAR_PATH;
        String body = buildBody(startName, startX, startY, endName, endX, endY);
        HttpResponse<String> response = sendPost(url, body);

        Preconditions.validate(
                response.statusCode() == 200, ErrorCode.SK_CAR_API_CALL_FAILED);

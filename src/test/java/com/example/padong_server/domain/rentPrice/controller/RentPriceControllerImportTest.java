...
class RentPriceControllerImportTest {
    ...
    void delegatesImportApiToImportService() {
        ResidencePriceImportResult expected = new ResidencePriceImportResult(1, 4L, 2L, 1L, 1L, 1L);
        when(rentPriceDataImportService.importData()).thenReturn(expected);

        ResidencePriceImportResult result = rentPriceController.importResidencePriceData().getBody();
